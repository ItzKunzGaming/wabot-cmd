	...
	// Filter Banned People
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai minna~, Saya ItzBot. Ketik ${prefix}menu untuk melihat menu bot`)
                      })
            } else {
                let cgrup = await aruga.getAllGroups()
                if (cgrup.length > groupLimit) return aruga.reply(from, `Maaf, grup bot penuh\nBatas grup bot adalah: ${groupLimit}`, id)
                if (cgrup.size < memberLimit) return aruga.reply(from, `Maaf, jumlah member kurang. Batas member grup harus ${memberLimit} orang`, id)
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () =>{
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *I T Z B O T B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *I T Z B O T  B C* 〙\n\n${msg}`)
            }
